...
The RPM spec file provided here is for Bitcoinold-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinold.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoinold git repository](https://github.com/bitcoinold/bitcoinold) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinold.conf` file.
...
in the bitcoinold git repository. At present only an SVG version of the Bitcoinold
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinold
...
needed when 0.12.1 is released, a proper fix is already in the Bitcoinold git
master branch.
...
idea to build Bitcoinold Core against a static version of that library compiled
...
The `bitcoinold.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinold-server` package. At this point, most RPM based
...
The `bitcoinold-server` package should have SELinux support. How to properly do
...
Bitcoinold-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinold/). He has been
packaging Bitcoinold for Fedora at least since 2012.
...
4. I renamed the `bitcoinold` package that contains the Qt GUI to `bitcoinold-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinold-xt` or `bitcoinold-classic`. I wanted to make sure the general population
