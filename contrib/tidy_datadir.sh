#!/bin/bash
# Copyright (c) 2013 The Bitcoinold Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitcoinold database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitcoinold datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitcoinold datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitcoinold 0.7 datadir."
    ;;
  3)
    echo "Detected Bitcoinold pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitcoinold 0.8 datadir."
    ;;
esac
