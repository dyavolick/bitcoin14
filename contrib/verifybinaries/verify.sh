...
###   This script attempts to download the signature file SHA256SUMS.asc from bitcoinold.org
...
WORKINGDIR="/tmp/bitcoinold_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://bitcoinold.org/bin/"
VERSIONPREFIX="bitcoinold-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinold-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinold Core binary release signing key installed?"
   fi
