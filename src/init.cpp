 ...
 * between bitcoinoldd, and bitcoinold-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bitcoinold-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BITCOINOLD_CONF_FILENAME));
    if (mode == HMM_BITCOINOLDD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), BITCOINOLD_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinold/bitcoinold>";
    const std::string URL_WEBSITE = "<https://bitcoinoldcore.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("bitcoinold-loadblk");
...
/** Sanity checks
 *  Ensure that Bitcoinold is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bitcoinold version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Bitcoinold process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", BITCOINOLD_CONF_FILENAME)).string());
